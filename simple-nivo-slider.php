...
function snivo_admin_init() {
	...
	register_setting('snivo-settings', 'snivo_posttype');
 ...
 */
function simple_nivo_slider($id='slider', $category='', $posttype='') {
	...
	# select proper post_type
	if (empty($posttype))
		$query .= '&post_type='.get_option('snivo_posttype', snivo_defaults('snivo_posttype'));
	else
		$query .= '&post_type='.$posttype;

	# select proper category
	if (empty($category))
		$query .= '&cat='.get_option('snivo_category', snivo_defaults('snivo_category'));
	else
...
function snivo_shortcode($atts) {
	...
	simple_nivo_slider($id, $category, $posttype);
...
function snivo_admin_options() {
	...
	?>
		...
		<form method="post" action="options.php" id="simple-nivo-slider-conf">
			...
			<tr><th scope="row">Post type</th><td>
			<select name="snivo_posttype">
				<option value="1">Select a post type</option> 
	 		<?php 
				$posttype = get_option('snivo_posttype', snivo_defaults('snivo_posttype'));
				$posttypes = array("post") + get_post_types(array('_builtin' => false), 'names'); 
				foreach ($posttypes as $type) {
					$typeobj = get_post_type_object($type);
					$typename = $typeobj->labels->singular_name;
			...
			<select name="snivo_category">
	 		...
	 		<?php 
				$category = get_option('snivo_category', snivo_defaults('snivo_category'));
